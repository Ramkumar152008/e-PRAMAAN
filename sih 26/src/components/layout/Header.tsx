    ...
    <>
      <header className="bg-[#0F2942] text-white sticky top-0 z-40 shadow-xs select-none border-b border-[#0A1D30]">
        ...
        <div className="px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          ...
          {/* Right: AI Assistant + Notifications + Profile Menu */}
          <div className="flex items-center gap-2.5">

            {/* Controlled Investigation AI Button */}
            {role === 'OFFICER' && (
              <button
                onClick={() => openAssistant()}
                className="flex items-center gap-1.5 px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded text-[11px] font-bold shadow-2xs border border-blue-400/40 transition cursor-pointer"
                title="Open AI Investigation Assistant"
              >
                <Scale className="w-3.5 h-3.5 text-amber-300" />
                <span className="hidden sm:inline">Investigation AI</span>
              </button>
            )}
